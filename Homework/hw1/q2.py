...
def server(params, opt, world):
    ...
    # usage: tranfer a list tensor to one 1-D tensor
...
def worker(params):
    ...
    flat_param_template = _flatten_dense_tensors([p.data for p in params]).contiguous()
    r = dist.irecv(flat_param_template, src=0)
    r.wait()
    
    synced_params = _unflatten_dense_tensors(flat_param_template, [p.data for p in params])
